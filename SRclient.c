...
//we need a variable to tell us that the last packet of data has been sent

struct TCP_PACKET rcv_buffer[5];
//need a structure to hold 5 packets at a time
//hold the packet, because it contains the sequence number of the data received

  //if the packets sequence number is < rcv_base, it is a duplicate
    //just send an ACK and do nothing to the buffer (possibly check for unwritten in order packets)
  
  //else, this is a new packet and we must loop through the buffer
    //look for a sequence number == rcv_base
        //if you find one write the data to the file and increment the rcv base by amount of data
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  //Assuming we'll receive the first packet indicating if the file was found correctly
  ...
  if (packet1.FILENOTFOUND_flag == 1) {
    fprintf(stderr,"404 NOT FOUND\n"); //display 404 not found
  }

  else (packet1.FILENOTFOUND_flag == 0) { //file was found and more packets might be coming
    ...
    write(fd,packet1.data,strlen(packet1.data)); //add first chunk of data

    //Initialize the Buffer
    for (int i = 0; i < 5; ++i) {
      rcv_buffer[i].seq_num=-1;
      bzero(rcv_buffer[i],DATA_SIZE);
    }
    ...
    while(numDataLastPacket == DATA_SIZE-1) { //indicating the last packet had full data packet, assuming at least one more packet left. Not gauranteed
